...
// CHECK: call void {{.*}}func3{{.*}}(ptr nocapture sret(%struct.S3) align 4 %agg.result, ptr noundef %agg.tmp)
struct S3 {
  int a[5];
};
...
// CHECK: define {{.*}} void {{.*}}f4{{.*}}(ptr inreg noalias nocapture sret(%struct.S4) align 4 %agg.result)
// CHECK: call void {{.*}}func4{{.*}}(ptr inreg nocapture sret(%struct.S4) align 4 %agg.result, [2 x i64] %0)
struct S4 {
  int a[3];
  ~S4();
};
...
// CHECK: define {{.*}} void @{{.*}}bar@Q1{{.*}}(ptr {{[^,]*}} %this, ptr inreg noalias nocapture sret(%class.P1) align 1 %agg.result)
// CHECK: call void {{.*}}foo@P1{{.*}}(ptr noundef{{[^,]*}} %ref.tmp, ptr inreg nocapture sret(%class.P1) align 1 %agg.result, i8 %0)
...
// CHECK: call void {{.*}}foo@P2{{.*}}(ptr noundef{{[^,]*}} %ref.tmp, ptr inreg nocapture sret(%class.P2) align 1 %retval, i8 %0)
class P2 {
public:
  P2 foo(P2 x);
};
...
// returned indirectly)
// CHECK: define {{.*}} void @{{.*}}f5{{.*}}(ptr inreg noalias nocapture sret(%struct.S5) align 4 %agg.result)
// CHECK: call void {{.*}}func5{{.*}}(ptr inreg nocapture sret(%struct.S5) align 4 %agg.result, i64 {{.*}})
struct S5 {
  S5();
  int x;
};
...
// CHECK: define {{.*}} void {{.*}}?f8{{.*}}(ptr inreg noalias nocapture sret(%struct.S8) align 4 {{.*}})
// CHECK: call void {{.*}}func8{{.*}}(ptr inreg nocapture sret(%struct.S8) align 4 {{.*}}, i64 {{.*}})
S8 func8(S8 x);
S8 f8() {
  S8 x;
  return func8(x);
...
// CHECK: define {{.*}} void @"?f9@@YA?AUS9@@XZ"(ptr inreg noalias nocapture sret(%struct.S9) align 4 {{.*}})
// CHECK: call void {{.*}}func9{{.*}}(ptr inreg nocapture sret(%struct.S9) align 4 {{.*}}, i64 {{.*}})
struct S9 {
...
// indirectly).
// CHECK: define dso_local void {{.*}}f10{{.*}}(ptr inreg noalias nocapture sret(%struct.S10) align 4 {{.*}})
// CHECK: call void {{.*}}func10{{.*}}(ptr inreg nocapture sret(%struct.S10) align 4 {{.*}}, [2 x i64] {{.*}})
...
// CHECK: define dso_local void {{.*}}f11{{.*}}(ptr inreg noalias nocapture sret(%struct.S11) align 8 {{.*}})
// CHECK: call void {{.*}}func11{{.*}}(ptr inreg nocapture sret(%struct.S11) align 8 {{.*}}, ptr {{.*}})
struct S11 {
  virtual void f();
  int a[5];
};
