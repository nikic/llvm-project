...
// GNU64: define dso_local void @TestLD(x86_fp80* noalias nocapture sret(x86_fp80) align 16 %agg.result, x86_fp80* noundef %0)
// MSC64: define dso_local double @TestLD(double noundef %x)

long double _Complex TestLDC(long double _Complex x) {
  return x * x;
}
// GNU32: define dso_local void @TestLDC({ x86_fp80, x86_fp80 }* noalias nocapture sret({ x86_fp80, x86_fp80 }) align 4 %agg.result, { x86_fp80, x86_fp80 }* noundef byval({ x86_fp80, x86_fp80 }) align 4 %x)
// GNU64: define dso_local void @TestLDC({ x86_fp80, x86_fp80 }* noalias nocapture sret({ x86_fp80, x86_fp80 }) align 16 %agg.result, { x86_fp80, x86_fp80 }* noundef %x)
// MSC64: define dso_local void @TestLDC({ double, double }* noalias nocapture sret({ double, double }) align 8 %agg.result, { double, double }* noundef %x)
