...
long double test_f128(long double f, long double g) {
  ...
  return f;
// CHECK: define{{.*}} void @test_f128(ptr noalias nocapture writeonly nocapture sret(fp128) align 8 [[DEST:%.*]], ptr nocapture noundef readonly %0, ptr nocapture noundef readonly %1)
