...
// CHECK: define{{.*}} void @f0(ptr noalias nocapture sret(%struct.int8x16x2_t) align 16 %agg.result, <16 x i8> noundef %{{.*}}, <16 x i8> noundef %{{.*}})
...
// CHECK: define{{.*}} void @f1_2(ptr noalias nocapture sret(<8 x float>) align 32 %{{.*}}, <8 x float> noundef %{{.*}})
T_float32x8 f1_2(T_float32x8 a0) { return a0; }
// CHECK: define{{.*}} void @f1_3(ptr noalias nocapture sret(<16 x float>) align 64 %{{.*}}, <16 x float> noundef %{{.*}})
