...
// PPC32LNX-LABEL:          define{{.*}} void @foo1(ptr noalias nocapture sret({ float, float }) align 4 %agg.result, ptr noundef byval({ float, float }) align 4 %x) #0 {
...
// PPC32LNX-LABEL:          define{{.*}} void @foo2(ptr noalias nocapture sret({ double, double }) align 8 %agg.result, ptr noundef byval({ double, double }) align 8 %x) #0 {
...
// PPC32LNX-LABEL:          define{{.*}} void @foo3(ptr noalias nocapture sret({ ppc_fp128, ppc_fp128 }) align 16 %agg.result, ptr noundef byval({ ppc_fp128, ppc_fp128 }) align 16 %x) #0 {
