...
// CHECK-LABEL: define{{.*}} void @_Z1fi(ptr noalias nocapture sret(%struct.coro) align 8 %agg.result, i32 noundef %0)
coro f(int) {
// CHECK: %call = call noalias noundef nonnull ptr @_Znwm(
// CHECK-NEXT: br label %[[CoroInit:.*]]

// CHECK: {{.*}}[[CoroInit]]:
// CHECK: call void @{{.*get_return_objectEv}}(ptr nocapture sret(%struct.coro) align 8 %agg.result
co_return;
}
...
// Verify that the RVO is applied.
// CHECK-LABEL: define{{.*}} void @_Z1hi(ptr noalias nocapture sret(%struct.coro_two) align 8 %agg.result, i32 noundef %0)
coro_two h(int) {
  ...
  // CHECK: {{.*}}[[InitOnFailure]]:
  // CHECK-NEXT: call void @{{.*get_return_object_on_allocation_failureEv}}(ptr nocapture sret(%struct.coro_two) align 8 %agg.result
  // CHECK-NEXT: br label %[[RetLabel:.*]]

  // CHECK: {{.*}}[[InitOnSuccess]]:
  // CHECK: call void @{{.*get_return_objectEv}}(ptr nocapture sret(%struct.coro_two) align 8 %agg.result
