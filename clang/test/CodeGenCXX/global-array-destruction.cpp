...
// CHECK: call {{.*}} @__cxa_atexit
// CHECK: getelementptr inbounds (%struct.T, %struct.T* bitcast ([2 x [3 x {{.*}}]]* @t to %struct.T*), i64 6)
...
// CHECK: call {{.*}} @__cxa_atexit
// CHECK: getelementptr inbounds (%struct.T, %struct.T* bitcast ([2 x [3 x {{.*}}]]* @_ZL2t2 to %struct.T*), i64 6)
// CHECK: call void @_ZN1TD1Ev
...
// CHECK: call {{.*}} @__cxa_atexit
// CHECK: getelementptr inbounds (%struct.T, %struct.T* getelementptr inbounds ([2 x [3 x {{.*}}]], [2 x [3 x {{.*}}]]* @_ZGR1u_, i32 0, i32 0, i32 0), i64 6)
// CHECK: call void @_ZN1TD1Ev
