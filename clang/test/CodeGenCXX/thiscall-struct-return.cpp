...
// CHECK: call x86_thiscallcc void @_ZNK1C5SmallEv(ptr nocapture sret(%struct.S) align 4 %{{.+}}, ptr {{[^,]*}} [[C]])
  (void)c.Small();
// CHECK: call x86_thiscallcc void @_ZNK1C6MediumEv(ptr nocapture sret(%struct.M) align 4 %{{.+}}, ptr {{[^,]*}} [[C]])
  (void)c.Medium();
}
