...
// CHECK:       (ptr inreg noundef %this, ptr inreg noalias nocapture sret(%struct.A) align 4 %agg.result, ptr inalloca(<{ %struct.A }>) %0)
// CHECK:   ret void
...
// CHECK:       (ptr inreg noundef %{{[^,]*}}, ptr inreg nocapture sret(%struct.A) align 4 %{{.*}}, ptr inalloca(<{ %struct.A }>) %{{[^,]*}})
