...
// WITH_NOALIAS: define{{.*}} void @_Z7CreateAPP1A(ptr noalias nocapture sret(%struct.A) align 1 %agg.result, ptr noundef %where)
// NO_NOALIAS: define{{.*}} void @_Z7CreateAPP1A(ptr noalias nocapture sret(%struct.A) align 1 %agg.result, ptr noundef %where)
A CreateA(A **where) {
  A justlikethis;
