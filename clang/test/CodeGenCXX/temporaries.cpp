...
namespace Elision {
  ...
  void test0() {
    ...
    // CHECK-NEXT: call void @_ZN7Elision4fooAEv(ptr nocapture sret([[A]]) align 8 [[T0]])
    ...
    // CHECK-NEXT: call void @_ZN7Elision4fooAEv(ptr nocapture sret([[A]]) align 8 [[K]])
  ...
  // CHECK: define{{.*}} void @_ZN7Elision5test2Ev(ptr noalias nocapture sret([[A]]) align 8
  A test2() {
  ...
  // CHECK: define{{.*}} void @_ZN7Elision5test3EiNS_1AE(ptr noalias nocapture sret([[A]]) align 8
  A test3(int v, A x) {
    if (v < 5)
  ...
  // CHECK: define{{.*}} void @_ZN7Elision5test5Ev(ptr noalias nocapture sret([[A]]) align 8
  struct B { A a; B(); };
  A test5() {
  ...
  void test6(const C *x) {
    ...
    // CHECK-NEXT: call void @_ZNK7Elision1CcvNS_1AEEv(ptr nocapture sret([[A]]) align 8 [[T0]], ptr {{[^,]*}} [[X]])
