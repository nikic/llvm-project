...
static bool ParseCodeGenArgs(CodeGenOptions &Opts, ArgList &Args, InputKind IK,
  ...
  if (Arg *FullRestrictArg =
          Args.getLastArg(OPT_ffull_restrict, OPT_fno_full_restrict)) {
    Opts.FullRestrict =
        FullRestrictArg->getOption().matches(OPT_ffull_restrict);
  }
  // Keep this behind an option - the alias analysis still works better
  // with the noalias attribute on arguments.
  Opts.NoNoAliasArgAttr = Args.hasArg(OPT_fno_noalias_arguments);
