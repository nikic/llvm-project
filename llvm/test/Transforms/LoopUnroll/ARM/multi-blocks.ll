...
; RUN: opt -mtriple=thumbv8m.main -mcpu=cortex-m33 -loop-unroll -S < %s -o - | FileCheck --check-prefixes=CHECK,CHECK-M33 %s
; RUN: opt -mtriple=thumbv7em -mcpu=cortex-m7 -loop-unroll -S < %s -o - | FileCheck --check-prefixes=CHECK,CHECK-M7 %s
...
; CHECK-NEXT:    br label [[FOR_BODY:%.*]]
; CHECK:       for.body:
; CHECK-NEXT:    [[J_016:%.*]] = phi i32 [ 0, [[FOR_BODY_PREHEADER_NEW]] ], [ [[INC_3:%.*]], [[IF_END_3:%.*]] ]
; CHECK-NEXT:    [[TEMP_015:%.*]] = phi i32 [ 0, [[FOR_BODY_PREHEADER_NEW]] ], [ [[TEMP_0_ADD_3:%.*]], [[IF_END_3]] ]
...
; CHECK-NEXT:    [[CMP1:%.*]] = icmp ugt i32 [[TMP2]], 65535
; CHECK-NEXT:    br i1 [[CMP1]], label [[CLEANUP_LOOPEXIT_EPILOG_LCSSA_LOOPEXIT:%.*]], label [[IF_END:%.*]]
; CHECK:       if.end:
...
; CHECK-NEXT:    [[ADD:%.*]] = select i1 [[TOBOOL]], i32 0, i32 [[TMP2]]
...
; CHECK-NEXT:    [[CMP1_1:%.*]] = icmp ugt i32 [[TMP4]], 65535
; CHECK-NEXT:    br i1 [[CMP1_1]], label [[CLEANUP_LOOPEXIT_EPILOG_LCSSA_LOOPEXIT]], label [[IF_END_1:%.*]]
; CHECK:       if.end.1:
; CHECK-NEXT:    [[ARRAYIDX2_1:%.*]] = getelementptr inbounds i32, i32* [[CONDITION]], i32 [[INC]]
; CHECK-NEXT:    [[TMP5:%.*]] = load i32, i32* [[ARRAYIDX2_1]], align 4
; CHECK-NEXT:    [[TOBOOL_1:%.*]] = icmp eq i32 [[TMP5]], 0
; CHECK-NEXT:    [[ADD_1:%.*]] = select i1 [[TOBOOL_1]], i32 0, i32 [[TMP4]]
...
; CHECK-NEXT:    [[CMP1_2:%.*]] = icmp ugt i32 [[TMP6]], 65535
; CHECK-NEXT:    br i1 [[CMP1_2]], label [[CLEANUP_LOOPEXIT_EPILOG_LCSSA_LOOPEXIT]], label [[IF_END_2:%.*]]
; CHECK:       if.end.2:
...
; CHECK-NEXT:    [[TOBOOL_2:%.*]] = icmp eq i32 [[TMP7]], 0
; CHECK-NEXT:    [[ADD_2:%.*]] = select i1 [[TOBOOL_2]], i32 0, i32 [[TMP6]]
; CHECK-NEXT:    [[TEMP_0_ADD_2:%.*]] = add i32 [[ADD_2]], [[TEMP_0_ADD_1]]
; CHECK-NEXT:    [[INC_2:%.*]] = add nuw nsw i32 [[INC_1]], 1
; CHECK-NEXT:    [[NITER_NEXT_2:%.*]] = add nuw nsw i32 [[NITER_NEXT_1]], 1
; CHECK-NEXT:    [[ARRAYIDX_3:%.*]] = getelementptr inbounds i32, i32* [[INPUT]], i32 [[INC_2]]
; CHECK-NEXT:    [[TMP8:%.*]] = load i32, i32* [[ARRAYIDX_3]], align 4
; CHECK-NEXT:    [[CMP1_3:%.*]] = icmp ugt i32 [[TMP8]], 65535
; CHECK-NEXT:    br i1 [[CMP1_3]], label [[CLEANUP_LOOPEXIT_EPILOG_LCSSA_LOOPEXIT]], label [[IF_END_3]]
; CHECK:       if.end.3:
; CHECK-NEXT:    [[ARRAYIDX2_3:%.*]] = getelementptr inbounds i32, i32* [[CONDITION]], i32 [[INC_2]]
; CHECK-NEXT:    [[TMP9:%.*]] = load i32, i32* [[ARRAYIDX2_3]], align 4
; CHECK-NEXT:    [[TOBOOL_3:%.*]] = icmp eq i32 [[TMP9]], 0
; CHECK-NEXT:    [[ADD_3:%.*]] = select i1 [[TOBOOL_3]], i32 0, i32 [[TMP8]]
; CHECK-NEXT:    [[TEMP_0_ADD_3]] = add i32 [[ADD_3]], [[TEMP_0_ADD_2]]
; CHECK-NEXT:    [[INC_3]] = add nuw i32 [[INC_2]], 1
; CHECK-NEXT:    [[NITER_NEXT_3]] = add i32 [[NITER_NEXT_2]], 1
; CHECK-NEXT:    [[NITER_NCMP_3:%.*]] = icmp ne i32 [[NITER_NEXT_3]], [[UNROLL_ITER]]
...
; CHECK-NEXT:    [[TEMP_0_LCSSA_PH:%.*]] = phi i32 [ [[TEMP_0_LCSSA_PH_PH]], [[CLEANUP_LOOPEXIT_UNR_LCSSA]] ], [ [[TEMP_0_LCSSA_PH_PH1]], [[CLEANUP_LOOPEXIT_EPILOG_LCSSA]] ]
...
;
; CHECK-M33-LABEL: @test_five_blocks(
; CHECK-M33-NEXT:  entry:
; CHECK-M33-NEXT:    [[CMP24:%.*]] = icmp ugt i32 [[MAXJ:%.*]], 1
; CHECK-M33-NEXT:    br i1 [[CMP24]], label [[FOR_BODY_PREHEADER:%.*]], label [[CLEANUP:%.*]]
; CHECK-M33:       for.body.preheader:
; CHECK-M33-NEXT:    [[TMP0:%.*]] = add i32 [[MAXJ]], -1
; CHECK-M33-NEXT:    [[TMP1:%.*]] = add i32 [[MAXJ]], -2
; CHECK-M33-NEXT:    [[XTRAITER:%.*]] = and i32 [[TMP0]], 3
; CHECK-M33-NEXT:    [[TMP2:%.*]] = icmp ult i32 [[TMP1]], 3
; CHECK-M33-NEXT:    br i1 [[TMP2]], label [[CLEANUP_LOOPEXIT_UNR_LCSSA:%.*]], label [[FOR_BODY_PREHEADER_NEW:%.*]]
; CHECK-M33:       for.body.preheader.new:
; CHECK-M33-NEXT:    [[UNROLL_ITER:%.*]] = sub i32 [[TMP0]], [[XTRAITER]]
; CHECK-M33-NEXT:    br label [[FOR_BODY:%.*]]
; CHECK-M33:       for.body:
; CHECK-M33-NEXT:    [[J_026:%.*]] = phi i32 [ 1, [[FOR_BODY_PREHEADER_NEW]] ], [ [[INC_3:%.*]], [[FOR_INC_3:%.*]] ]
; CHECK-M33-NEXT:    [[TEMP_025:%.*]] = phi i32 [ 0, [[FOR_BODY_PREHEADER_NEW]] ], [ [[TEMP_1_3:%.*]], [[FOR_INC_3]] ]
; CHECK-M33-NEXT:    [[NITER:%.*]] = phi i32 [ 0, [[FOR_BODY_PREHEADER_NEW]] ], [ [[NITER_NEXT_3:%.*]], [[FOR_INC_3]] ]
; CHECK-M33-NEXT:    [[ARRAYIDX:%.*]] = getelementptr inbounds i32, i32* [[INPUT:%.*]], i32 [[J_026]]
; CHECK-M33-NEXT:    [[TMP3:%.*]] = load i32, i32* [[ARRAYIDX]], align 4
; CHECK-M33-NEXT:    [[ADD:%.*]] = add i32 [[TMP3]], [[TEMP_025]]
; CHECK-M33-NEXT:    [[CMP1:%.*]] = icmp ugt i32 [[ADD]], 16777215
; CHECK-M33-NEXT:    br i1 [[CMP1]], label [[CLEANUP_LOOPEXIT_EPILOG_LCSSA_LOOPEXIT:%.*]], label [[IF_END:%.*]]
; CHECK-M33:       if.end:
; CHECK-M33-NEXT:    [[ARRAYIDX2:%.*]] = getelementptr inbounds i32, i32* [[CONDITION:%.*]], i32 [[J_026]]
; CHECK-M33-NEXT:    [[TMP4:%.*]] = load i32, i32* [[ARRAYIDX2]], align 4
; CHECK-M33-NEXT:    [[CMP3:%.*]] = icmp ugt i32 [[TMP4]], 65535
; CHECK-M33-NEXT:    br i1 [[CMP3]], label [[IF_THEN4:%.*]], label [[IF_ELSE:%.*]]
; CHECK-M33:       if.then4:
; CHECK-M33-NEXT:    [[SUB:%.*]] = add nsw i32 [[J_026]], -1
; CHECK-M33-NEXT:    [[ARRAYIDX6:%.*]] = getelementptr inbounds i32, i32* [[INPUT]], i32 [[SUB]]
; CHECK-M33-NEXT:    [[TMP5:%.*]] = load i32, i32* [[ARRAYIDX6]], align 4
; CHECK-M33-NEXT:    [[CMP7:%.*]] = icmp ugt i32 [[TMP3]], [[TMP5]]
; CHECK-M33-NEXT:    [[COND:%.*]] = zext i1 [[CMP7]] to i32
; CHECK-M33-NEXT:    [[ADD8:%.*]] = add i32 [[ADD]], [[COND]]
; CHECK-M33-NEXT:    br label [[FOR_INC:%.*]]
; CHECK-M33:       if.else:
; CHECK-M33-NEXT:    [[AND:%.*]] = and i32 [[ADD]], [[TMP3]]
; CHECK-M33-NEXT:    br label [[FOR_INC]]
; CHECK-M33:       for.inc:
; CHECK-M33-NEXT:    [[TEMP_1:%.*]] = phi i32 [ [[ADD8]], [[IF_THEN4]] ], [ [[AND]], [[IF_ELSE]] ]
; CHECK-M33-NEXT:    [[INC:%.*]] = add nuw nsw i32 [[J_026]], 1
; CHECK-M33-NEXT:    [[NITER_NEXT:%.*]] = add nuw nsw i32 [[NITER]], 1
