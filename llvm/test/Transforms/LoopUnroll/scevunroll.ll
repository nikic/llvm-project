...
; CHECK-NEXT:    br i1 true, label [[TAIL:%.*]], label [[EXIT1:%.*]]
; CHECK:       tail:
; CHECK-NEXT:    [[CMP2:%.*]] = icmp ne i64 [[VAL]], 0
; CHECK-NEXT:    br i1 [[CMP2]], label [[LOOP_1:%.*]], label [[EXIT2:%.*]]
; CHECK:       exit1:
; CHECK-NEXT:    [[S_LCSSA:%.*]] = phi i64 [ 0, [[LOOP]] ], [ [[VAL]], [[LOOP_1]] ], [ [[S_NEXT_1:%.*]], [[LOOP_2:%.*]] ], [ [[S_NEXT_2:%.*]], [[LOOP_3:%.*]] ]
...
; CHECK-NEXT:    br i1 true, label [[TAIL_1]], label [[EXIT1]]
; CHECK:       tail.1:
...
; CHECK-NEXT:    br i1 true, label [[TAIL_2]], label [[EXIT1]]
; CHECK:       tail.2:
...
; CHECK:       l1:
; CHECK-NEXT:    [[VAL:%.*]] = load i32, i32* [[BASE:%.*]], align 4
; CHECK-NEXT:    br i1 true, label [[L2:%.*]], label [[EXIT1:%.*]]
; CHECK:       l2:
...
; CHECK-NEXT:    br i1 true, label [[L2_1:%.*]], label [[EXIT1]]
; CHECK:       l2.1:
...
; CHECK-NEXT:    br i1 true, label [[L2_2:%.*]], label [[EXIT1]]
; CHECK:       l2.2:
...
; CHECK-NEXT:    br i1 true, label [[L2_3:%.*]], label [[EXIT1]]
; CHECK:       l2.3:
...
; CHECK-NEXT:    br i1 true, label [[L2_4:%.*]], label [[EXIT1]]
; CHECK:       l2.4:
...
; CHECK:       for.body:
; CHECK-NEXT:    br i1 true, label [[FOR_COND:%.*]], label [[RETURN:%.*]]
; CHECK:       for.cond:
; CHECK-NEXT:    br i1 false, label [[RETURN]], label [[FOR_BODY_1:%.*]]
; CHECK:       return:
; CHECK-NEXT:    [[B_03_LCSSA:%.*]] = phi i32 [ 8, [[FOR_BODY]] ], [ 0, [[FOR_COND]] ], [ 8, [[FOR_BODY_1]] ], [ 0, [[FOR_COND_1:%.*]] ]
; CHECK-NEXT:    [[RETVAL_0:%.*]] = phi i32 [ 1, [[FOR_BODY]] ], [ 0, [[FOR_COND]] ], [ 1, [[FOR_BODY_1]] ], [ 0, [[FOR_COND_1]] ]
