...
; %p may alias errno, but cosf does not clobber errno.
; Can do constant store-to-load forwarding.
define float @may_alias_errno_does_not_clobber(ptr %p, float %f) {
; CHECK-LABEL: define float @may_alias_errno_does_not_clobber(
; CHECK-SAME: ptr [[P:%.*]], float [[F:%.*]]) {
; CHECK-NEXT:  [[ENTRY:.*:]]
; CHECK-NEXT:    store float 0.000000e+00, ptr [[P]], align 4
; CHECK-NEXT:    [[CALL:%.*]] = call float @cosf(float [[F]])
; CHECK-NEXT:    ret float 0.000000e+00
;
entry:
  store float 0.000000e+00, ptr %p, align 4
  %call = call float @cosf(float %f)
...
declare float @cosf(float) memory(argmem: write, errnomem: read)
