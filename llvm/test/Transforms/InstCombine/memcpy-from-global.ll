...
; CHECK-NEXT:    [[T1851:%.*]] = getelementptr float, ptr @C.0.1248, i64 [[TMP0]]
; CHECK-NEXT:    [[T19:%.*]] = load float, ptr [[T1851]], align 4
; CHECK-NEXT:    [[T21:%.*]] = fmul float [[T19]], [[Y:%.*]]
; CHECK-NEXT:    [[T13:%.*]] = fadd float [[T21]], 0.000000e+00
; CHECK-NEXT:    [[TMP1:%.*]] = zext nneg i32 [[T5]] to i64
; CHECK-NEXT:    [[TMP3:%.*]] = getelementptr float, ptr @C.0.1248, i64 [[TMP1]]
; CHECK-NEXT:    [[T1852:%.*]] = getelementptr i8, ptr [[TMP3]], i64 4
; CHECK-NEXT:    [[T20:%.*]] = load float, ptr [[T1852]], align 4
; CHECK-NEXT:    [[T22:%.*]] = fmul float [[T20]], [[Y1:%.*]]
; CHECK-NEXT:    [[T23:%.*]] = fadd float [[T22]], [[T13]]
; CHECK-NEXT:    [[TMP2:%.*]] = zext nneg i32 [[T5]] to i64
; CHECK-NEXT:    [[TMP5:%.*]] = getelementptr float, ptr @C.0.1248, i64 [[TMP2]]
; CHECK-NEXT:    [[T2849:%.*]] = getelementptr i8, ptr [[TMP5]], i64 8
; CHECK-NEXT:    [[T29:%.*]] = load float, ptr [[T2849]], align 4
; CHECK-NEXT:    [[T31:%.*]] = fmul float [[T29]], [[Z:%.*]]
; CHECK-NEXT:    [[T33:%.*]] = fadd float [[T31]], [[T23]]
; CHECK-NEXT:    [[TMP4:%.*]] = zext nneg i32 [[T5]] to i64
; CHECK-NEXT:    [[TMP6:%.*]] = getelementptr float, ptr @C.0.1248, i64 [[TMP4]]
