...
; A readnone may-throw call cannot be hoisted if preceded by a store.
; Hoisting would skip the side effect that must happen before the throw.
define void @no_hoist_maythrow_after_store(i32 %n, ptr %p, ptr noalias %sink) {
; CHECK-LABEL: define void @no_hoist_maythrow_after_store(
; CHECK-SAME: i32 [[N:%.*]], ptr [[P:%.*]], ptr noalias [[SINK:%.*]]) {
; CHECK-NEXT:  [[ENTRY:.*]]:
; CHECK-NEXT:    br label %[[LOOP:.*]]
; CHECK:       [[LOOP]]:
; CHECK-NEXT:    [[IV:%.*]] = phi i32 [ 0, %[[ENTRY]] ], [ [[IV_NEXT:%.*]], %[[LOOP]] ]
...
; A readnone may-throw call cannot be hoisted if preceded by a side-effectful
; nounwind call. The side effect must happen before the potential throw.
declare void @nounwind_sideeffect() nounwind

define void @no_hoist_maythrow_after_sideeffect(i32 %n, ptr noalias %sink) {
; CHECK-LABEL: define void @no_hoist_maythrow_after_sideeffect(
; CHECK-SAME: i32 [[N:%.*]], ptr noalias [[SINK:%.*]]) {
; CHECK-NEXT:  [[ENTRY:.*]]:
; CHECK-NEXT:    br label %[[LOOP:.*]]
; CHECK:       [[LOOP]]:
; CHECK-NEXT:    [[IV:%.*]] = phi i32 [ 0, %[[ENTRY]] ], [ [[IV_NEXT:%.*]], %[[LOOP]] ]
; CHECK-NEXT:    call void @nounwind_sideeffect()
; CHECK-NEXT:    [[RET:%.*]] = call i32 @readnone_maythrow(i32 [[N]])
; CHECK-NEXT:    store volatile i32 [[RET]], ptr [[SINK]], align 4
; CHECK-NEXT:    [[IV_NEXT]] = add i32 [[IV]], 1
; CHECK-NEXT:    [[CMP:%.*]] = icmp slt i32 [[IV]], 200
