...
; CHECK-NEXT:  Loop %for.body: backedge-taken count is (((-2 smax (1 + (-1 * %len))) + %len) /u 2)
; CHECK-NEXT:  Loop %for.body: constant max backedge-taken count is i32 1073741823
; CHECK-NEXT:  Loop %for.body: symbolic max backedge-taken count is (((-2 smax (1 + (-1 * %len))) + %len) /u 2)
