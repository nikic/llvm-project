...
; CHECK-NEXT:    --> (select %cond_i, %cond_i2, false) U: full-set S: full-set Exits: <<Unknown>> LoopDispositions: { %loop: Variant }
...
; CHECK-NEXT:    %cond = select i1 %cond_i, i1 %cond_i2, i1 false
; CHECK-NEXT:    --> (select %cond_i, %cond_i2, false) U: full-set S: full-set Exits: false LoopDispositions: { %loop: Variant }
...
; CHECK-NEXT:    --> (select %cond_i, true, %cond_i2) U: full-set S: full-set Exits: <<Unknown>> LoopDispositions: { %loop: Variant }
...
; CHECK-NEXT:    --> {1,+,1}<%loop> U: [1,2) S: [1,2) Exits: 1 LoopDispositions: { %loop: Computable }
; CHECK-NEXT:    %cond = select i1 %cond_i, i1 true, i1 %cond_i2
; CHECK-NEXT:    --> (select %cond_i, true, %cond_i2) U: full-set S: full-set Exits: true LoopDispositions: { %loop: Variant }
