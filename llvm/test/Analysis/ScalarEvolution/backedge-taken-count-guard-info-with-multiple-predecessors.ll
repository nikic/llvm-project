...
; CHECK-NEXT:  Loop %while.body: backedge-taken count is (((-8 umax (7 + (-1 * %count))) + %count) /u 8)
; CHECK-NEXT:  Loop %while.body: constant max backedge-taken count is i64 2305843009213693951
; CHECK-NEXT:  Loop %while.body: symbolic max backedge-taken count is (((-8 umax (7 + (-1 * %count))) + %count) /u 8)
...
; CHECK-LABEL: 'hang_due_to_unreachable_phi_inblock'
; CHECK-NEXT:  Determining loop execution counts for: @hang_due_to_unreachable_phi_inblock
; CHECK-NEXT:  Loop %bb8: backedge-taken count is i1 false
; CHECK-NEXT:  Loop %bb8: constant max backedge-taken count is i1 false
; CHECK-NEXT:  Loop %bb8: symbolic max backedge-taken count is i1 false
; CHECK-NEXT:  Loop %bb8: Trip multiple is 1
;
bb:
  br label %bb6
