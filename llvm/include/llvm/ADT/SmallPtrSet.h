...
public:
  ...
  void remove_if(UnaryPredicate P) {
    if (isSmall()) {
      const void **Dst = SmallArray, **Src = SmallArray;
      const void **End = SmallArray + NumNonEmpty;
      for (; Src != End; ++Src) {
        PtrType Ptr = PtrTraits::getFromVoidPointer(const_cast<void *>(*Src));
        if (!P(Ptr))
          *Dst++ = *Src;
      }
      NumNonEmpty = Dst - SmallArray;
      return;
    }
