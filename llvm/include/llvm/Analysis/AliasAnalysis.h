...
private:
  ...
  FunctionModRefBehavior(uint32_t Data = 0) : Data(Data) {}
...
public:
  ...
  /// given ModRefInfo.
  static FunctionModRefBehavior anyLoc(ModRefInfo MR) {
    FunctionModRefBehavior FRMB;
    for (Location Loc : locations())
      FRMB.setModRef(Loc, MR);
    return FRMB;
  }
  ...
  /// Set ModRefInfo for the given Location.
  ...
  bool onlyAccessesArgPointees() const {
    return withoutLoc(ArgMem).doesNotAccessMemory();
  }
  ...
  bool onlyAccessesInaccessibleMem() const {
    return withoutLoc(InaccessibleMem).doesNotAccessMemory();
  }
