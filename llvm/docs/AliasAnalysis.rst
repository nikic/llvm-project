...
(functions, constant global variables, and the null pointer). The
...
locally-invariant memory (for which it returns ``Ref``). Locally-invariant
memory is memory that we know is invariant for the lifetime of its SSA value,
but not necessarily for the life of the program: for example, the memory
pointed to by ``readonly`` ``noalias`` parameters is known-invariant for the
duration of the corresponding function call. Given Mod/Ref information
``MRI`` for a memory location ``Loc``, ``MRI`` can be refined with a
statement like ``MRI &= AA.getModRefInfoMask(Loc);``. Another useful idiom is
``isModSet(AA.getModRefInfoMask(Loc))``; this checks to see if the given
location can be modified at all.
