...
struct TestVisitor : public PtrUseVisitor<TestVisitor> {
  ...
  void visitStoreInst(StoreInst &SI) {
    Visits.push_back({&SI, IsOffsetKnown, Offset, PHIsInPath});
    if (SI.getValueOperand() == U->get())
      PI.setEscaped(&SI);
  }

  void visitLoadInst(LoadInst &LI) {
    Visits.push_back({&LI, IsOffsetKnown, Offset, PHIsInPath});
  }
};
...
TEST(PtrUseVisitorTest, PHIWithConflictingOffsets) {
  ...
  EXPECT_EQ(TV.Visits[0].PHIsInPath.size(), 1u);
  EXPECT_TRUE(TV.Visits[0].PHIsInPath.contains(Phi));
  EXPECT_EQ(TV.Visits[1].PHIsInPath.size(), 1u);
  EXPECT_TRUE(TV.Visits[1].PHIsInPath.contains(Phi));
}
...
TEST(PtrUseVisitorTest, PHITracking) {
  ...
  // Check that both visits tracked the PHI node with its offset
  ASSERT_EQ(TV.Visits.size(), 2u);
  EXPECT_EQ(TV.Visits[0].PHIsInPath.size(), 1u);
  EXPECT_EQ(TV.Visits[1].PHIsInPath.size(), 1u);
  EXPECT_EQ(TV.Visits[0].PHIsInPath.count(Phi), 1u);
  EXPECT_EQ(TV.Visits[1].PHIsInPath.count(Phi), 1u);
  // Check that the PHI was recorded with the correct offsets (0 and 8)
  EXPECT_TRUE(
      (TV.Visits[0].PHIsInPath[Phi] == 0 &&
       TV.Visits[1].PHIsInPath[Phi] == 8) ||
      (TV.Visits[0].PHIsInPath[Phi] == 8 && TV.Visits[1].PHIsInPath[Phi] == 0));
}
