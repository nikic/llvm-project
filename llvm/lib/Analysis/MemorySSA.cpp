    ...
    for (const auto *MA : def_chain(MAP.MA)) {
      if (MA == ClobberAt) {
        if (const auto *MD = dyn_cast<MemoryDef>(MA)) {
          ...
          if (!FoundClobber) {
            BatchAACrossIterationScope _(AA, MAP.MayHaveLoopCarriedDependency);
...
class ClobberWalker {
  ...
  walkToPhiOrClobber(DefPath &Desc, const MemoryAccess *StopAt = nullptr,
    ...
    for (MemoryAccess *Current : def_chain(Desc.Last)) {
      ...
      if (auto *MD = dyn_cast<MemoryDef>(Current)) {
        ...
        BatchAACrossIterationScope _(*AA, Desc.MayHaveLoopCarriedDependency);
  ...
  void addSearches(MemoryPhi *Phi, SmallVectorImpl<ListIndex> &PausedSearches,
                   ListIndex PriorNode, bool MayHaveLoopCarriedDependency) {
    auto UpwardDefsBegin = upward_defs_begin(
        {Phi, Paths[PriorNode].Loc, MayHaveLoopCarriedDependency}, DT);
    ...
    for (const UpwardDefsElem &E : UpwardDefs) {
      ...
      Paths.emplace_back(E.Loc, E.MA, E.MayHaveLoopCarriedDependency,
                         PriorNode);
    }
  }
  ...
  getBlockingAccess(const MemoryAccess *StopWhere,
    ...
    while (!PausedSearches.empty()) {
      ...
      // work.
      if (!VisitedPhis
               .insert({Node.Last, Node.Loc, Node.MayHaveLoopCarriedDependency})
               .second)
        continue;
      ...
      addSearches(cast<MemoryPhi>(Res.Result), PausedSearches, PathIndex,
                  Node.MayHaveLoopCarriedDependency);
    }
    ...
    while (true) {
      ...
      for (ListIndex I : NewPaused)
        addSearches(DefChainPhi, PausedSearches, I,
                    Paths[I].MayHaveLoopCarriedDependency);
      NewPaused.clear();
