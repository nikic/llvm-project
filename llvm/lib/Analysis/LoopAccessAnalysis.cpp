...
/// uniformity and if it contains UDiv expressions.
class SCEVAddRecForUniformityRewriter
  ...
  /// Set to true if the expression contains an UDiv expression. Uniform values
...
public:
  ...
  const SCEV *visitUDivExpr(const SCEVUDivExpr *S) {
    FoundUDiv = true;
    return SCEVRewriteVisitor<SCEVAddRecForUniformityRewriter>::visitUDivExpr(
        S);
  }
