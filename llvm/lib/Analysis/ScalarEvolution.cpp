...
static cl::opt<unsigned>
TinyExprThreshold("scalar-evolution-tiny-expr-threshold", cl::Hidden,
                  cl::desc("Size of the expression which is considered tiny "
                           "and cheap enough for extra-agressive analysis"),
                  cl::init(6));
...
ScalarEvolution::getRangeRef(const SCEV *S,
  ...
  if (const SCEVAddRecExpr *AddRec = dyn_cast<SCEVAddRecExpr>(S)) {
    ...
    if (AddRec->isAffine()) {
      ...
      MaxBECount = getSymbolicMaxBackedgeTakenCount(AddRec->getLoop());
      if (!isa<SCEVCouldNotCompute>(MaxBECount) &&
          getTypeSizeInBits(MaxBECount->getType()) <= BitWidth &&
...
static bool isTinyExpr(const SCEV *S) {
  return S->getExpressionSize() <= TinyExprThreshold;
}
...
ConstantRange ScalarEvolution::getRangeForAffineNoSelfWrappingAR(
  ...
  // To save some compile time, we want to limit our analysis to tiny BE Count
  // and Start, because they participate in End computation and we want to avoid
  // heavy transforms there.
  const SCEV *Start = AddRec->getStart();
  if (!isTinyExpr(Start) || !isTinyExpr(MaxBECount))
    return ConstantRange::getFull(BitWidth);
