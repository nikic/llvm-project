  ...
  if (const SCEVConstant *LHSC = dyn_cast<SCEVConstant>(Ops[0])) {
    if (Ops.size() == 2) {
      ...
      if (const SCEVAddExpr *Add = dyn_cast<SCEVAddExpr>(Ops[1])) {
        auto IsFoldingProfitable = [Add]() {
          // Only distribute the constant if there is at most one operand that
          // does not have a constant factor.
          bool HasOpWithoutConstant = false;
          for (const SCEV *Op : Add->operands()) {
            if (isa<SCEVConstant>(Op))
              continue;
            if (const auto *Mul = dyn_cast<SCEVMulExpr>(Op))
              if (isa<SCEVConstant>(Mul->getOperand(0)))
                continue;
            if (HasOpWithoutConstant)
              return false;
            HasOpWithoutConstant = true;
          }
          return true;
        };
        if (IsFoldingProfitable()) {
          ...
          for (const SCEV *AddOp : Add->operands())
            NewOps.push_back(getMulExpr(LHSC, AddOp, SCEV::FlagAnyWrap,
                                        Depth + 1));
