...
getPredicateResult(unsigned Pred, Constant *C, const ValueLatticeElement &Val,
  ...
  ValueLatticeElement Other = ValueLatticeElement::get(C);
  Type *Ty = Type::getInt1Ty(C->getContext());
  if (Constant *Res = Val.getCompare((CmpInst::Predicate)Pred, Ty, Other, DL)) {
    if (Res->isNullValue())
