...
LLVMAttributeRef LLVMCreateStringAttribute(LLVMContextRef C,
  ...
  return wrap(Attribute::get(*unwrap(C),
                             AttributeKey::Create(StringRef(K, KLength)),
                             StringRef(V, VLength)));
}

const char *LLVMGetStringAttributeKind(LLVMAttributeRef A,
                                       unsigned *Length) {
  auto S = unwrap(A).getKindAsKey().value();
  *Length = S.size();
  return S.data();
}
...
LLVMAttributeRef LLVMGetStringAttributeAtIndex(LLVMValueRef F,
  ...
  return wrap(unwrap<Function>(F)->getAttributeAtIndex(
      Idx, AttributeKey::Create(StringRef(K, KLen))));
}
...
void LLVMRemoveStringAttributeAtIndex(LLVMValueRef F, LLVMAttributeIndex Idx,
                                      const char *K, unsigned KLen) {
  unwrap<Function>(F)->removeAttributeAtIndex(
      Idx, AttributeKey::Create(StringRef(K, KLen)));
}
...
void LLVMAddTargetDependentFunctionAttr(LLVMValueRef Fn, const char *A,
  ...
  Function *Func = unwrap<Function>(Fn);
  Attribute Attr =
      Attribute::get(Func->getContext(), AttributeKey::Create(A), V);
  Func->addFnAttr(Attr);
}
...
void LLVMRemoveCallSiteStringAttribute(LLVMValueRef C, LLVMAttributeIndex Idx,
                                       const char *K, unsigned KLen) {
  unwrap<CallBase>(C)->removeAttributeAtIndex(
      Idx, AttributeKey::Create(StringRef(K, KLen)));
}
