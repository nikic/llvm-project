...
void Verifier::verifyFunctionAttrs(FunctionType *FT, AttributeList Attrs,
  ...
  if (Attrs.hasFnAttr(FramePointerAttr)) {
    StringRef FP = Attrs.getFnAttr(FramePointerAttr).getValueAsString();
    if (FP != "all" && FP != "non-leaf" && FP != "none")
      CheckFailed("invalid value for 'frame-pointer' attribute: " + FP, V);
  }

  checkUnsignedBaseTenFuncAttr(Attrs, PatchableFunctionPrefixAttr, V);
  checkUnsignedBaseTenFuncAttr(Attrs, PatchableFunctionEntryAttr, V);
  checkUnsignedBaseTenFuncAttr(Attrs, WarnStackSizeAttr, V);
}
