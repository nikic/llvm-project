...
template <> bool MachineCycle::isCycleInvariant(MachineInstr &I) const {
  ...
  for (const MachineOperand &MO : I.operands()) {
    ...
    if (Register::isPhysicalRegister(Reg)) {
      if (MO.isUse()) {
        ...
        return false;
      } else if (any_of(getHeaders(), [&](const MachineBasicBlock *Block) {
    ...
    if (contains(MRI->getVRegDef(Reg)->getParent()))
      return false;
  }
