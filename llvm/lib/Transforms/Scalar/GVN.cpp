...
static Value *findDominatingValue(const MemoryLocation &Loc, Instruction *From,
                                  AAResults *AA) {
...
GVNPass::AnalyzeLoadAvailability(LoadInst *Load, MemDepResult DepInfo,
  ...
  if (auto *Sel = dyn_cast<SelectInst>(DepInst)) {
    ...
    Value *V1 = findDominatingValue(Loc.getWithNewPtr(Sel->getTrueValue()),
                                    DepInst, getAliasAnalysis());
    if (!V1)
      return std::nullopt;
    Value *V2 = findDominatingValue(Loc.getWithNewPtr(Sel->getFalseValue()),
                                    DepInst, getAliasAnalysis());
    if (!V2)
      return std::nullopt;
