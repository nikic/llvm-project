...
namespace {
struct InferAlignmentLegacyPass : public FunctionPass {
  static char ID;
  InferAlignmentLegacyPass() : FunctionPass(ID) {
    initializeInferAlignmentLegacyPassPass(*PassRegistry::getPassRegistry());
  }
  ...
  bool runOnFunction(Function &F) override {
    ...
    return inferAlignment(F, AC, DT);
  }
};
}

char InferAlignmentLegacyPass::ID = 0;
INITIALIZE_PASS_BEGIN(InferAlignmentLegacyPass, "infer-alignment",
                      "Infer alignment", false, false)
INITIALIZE_PASS_DEPENDENCY(AssumptionCacheTracker)
INITIALIZE_PASS_DEPENDENCY(DominatorTreeWrapperPass)
INITIALIZE_PASS_END(InferAlignmentLegacyPass, "infer-alignment",
                    "Infer alignment", false, false)

FunctionPass *llvm::createInferAlignmentLegacyPass() {
  return new InferAlignmentLegacyPass();
}
