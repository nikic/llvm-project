...
static bool mergeIdenticalBlocks(Function &F) {
  ...
  for (BasicBlock &BB : F) {
    ...
    SameHashBlocks.try_emplace(hashBlock(BB)).first->second.push_back(&BB);
  }

  // Try a pair-wise merge of blocks with the same hash.
  bool Changed = false;
  for (auto &Pair : SameHashBlocks) {
    auto &Blocks = Pair.second;
    for (auto I1 = Blocks.begin(); I1 < Blocks.end(); ++I1) {
      auto I2 = I1 + 1;
      while (I2 < Blocks.end()) {
        if (tryMergeTwoBlocks(**I1, **I2)) {
          Blocks.erase(I2);
          ++NumBlocksMerged;
          Changed = true;
        } else {
