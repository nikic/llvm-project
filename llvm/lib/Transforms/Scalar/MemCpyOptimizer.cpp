...
static cl::opt<unsigned> MemCpyOptStackMoveThreshold(
...
private:
  ...
  void getAnalysisUsage(AnalysisUsage &AU) const override {
    ...
    // If we aren't doing the stack-move optimization, we don't need the
    // postdominator tree for anything.
...
bool MemCpyOptPass::processStore(StoreInst *SI, BasicBlock::iterator &BBI) {
  ...
  if (auto *LI = dyn_cast<LoadInst>(StoredVal)) {
    if (LI->isSimple() && LI->hasOneUse() &&
      ...
      if (MemCpyOptStackMoveThreshold > 0) {
        if (AllocaInst *DestAlloca = dyn_cast<AllocaInst>(
                SI->getPointerOperand()->stripPointerCastsAndAliases())) {
...
bool MemCpyOptPass::performStackMoveOptzn(Instruction *Load, Instruction *Store,
                                          ...
                                          uint64_t Size) {
  assert(MemCpyOptStackMoveThreshold > 0 &&
         "Shouldn't have called performStackMoveOptzn() if stack-move "
  ...
  if (!SizeOK) {
    LLVM_DEBUG(dbgs() << "Stack Move: Size mismatch\n");
    return false;
  }
  ...
  if (DestTracker.Abort) {
    LLVM_DEBUG(dbgs() << "Stack Move: Destination was captured:");
  ...
  // We can do the transformation. First, merge the two allocas.
...
bool MemCpyOptPass::processMemCpy(MemCpyInst *M, BasicBlock::iterator &BBI) {
  ...
  if (MemCpyOptStackMoveThreshold > 0) {
    if (AllocaInst *DestAlloca =
      ...
      if (AllocaInst *SrcAlloca = dyn_cast<AllocaInst>(
              M->getSource()->stripPointerCastsAndAliases())) {
