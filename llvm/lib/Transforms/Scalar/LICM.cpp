...
collectPromotionCandidates(MemorySSA *MSSA, AliasAnalysis *AA, Loop *L) {
  ...
  foreachMemoryAccess(MSSA, L, [&](Instruction *I) {
    ...
    llvm::erase_if(Sets, [&](std::pair<const AliasSet *, bool> &Pair) {
      ModRefInfo MR = Pair.first->aliasesUnknownInst(I, BatchAA);
      // Cannot promote if there are writes outside the set.
      if (isModSet(MR))
        return true;
      // Remember reads outside the set.
      Pair.second |= isRefSet(MR);
      // If this is a mod-only set and there are reads outside the set, we will
      // not be able to promote, so bail out early.
      return !Pair.first->isRef() && Pair.second;
    });
  });
