...
static bool isSafeToExecuteUnconditionally(
    ...
    OptimizationRemarkEmitter *ORE, const Instruction *CtxI, bool Speculate);
...
struct LegacyLICMPass : public LoopPass {
  ...
  LegacyLICMPass(
      ...
      bool LicmSpeculate = true)
      : LoopPass(ID),
        LICM(LicmMssaOptCap, LicmMssaNoAccForPromotionCap, LicmSpeculate) {
      ...
      do {
        ...
        for (const SmallSetVector<Value *, 8> &PointerMustAliases :
          ...
          LocalPromoted |= promoteLoopAccessesToScalars(
              ...
              DT, TLI, L, &MSSAU, &SafetyInfo, ORE, LicmSpeculate);
        }
    ...
    for (Instruction &I : llvm::make_early_inc_range(*BB)) {
      ...
      if (CurLoop->hasLoopInvariantOperands(&I) &&
          ...
          isSafeToExecuteUnconditionally(
              ...
              CurLoop->getLoopPreheader()->getTerminator(), Speculate)) {
...
static bool isSafeToExecuteUnconditionally(
    ...
    const Loop *CurLoop, const LoopSafetyInfo *SafetyInfo,
    OptimizationRemarkEmitter *ORE, const Instruction *CtxI, bool Speculate) {
  if (Speculate && isSafeToSpeculativelyExecute(&Inst, CtxI, DT, TLI))
    return true;
...
bool llvm::promoteLoopAccessesToScalars(
    ...
    Loop *CurLoop, MemorySSAUpdater *MSSAU, ICFLoopSafetyInfo *SafetyInfo,
    OptimizationRemarkEmitter *ORE, bool Speculate) {
  // Verify inputs.
        ...
        if (!DereferenceableInPH || (InstAlignment > Alignment))
          if (isSafeToExecuteUnconditionally(
                  *Load, DT, TLI, CurLoop, SafetyInfo, ORE,
                  Preheader->getTerminator(), Speculate)) {
