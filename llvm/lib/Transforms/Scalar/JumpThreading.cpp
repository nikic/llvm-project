...
/// when exceeding the threshold. If duplication is impossible, returns invalid
/// cost.
...
getJumpThreadDuplicationCost(const TargetTransformInfo *TTI, BasicBlock *BB,
  ...
  for (; &*I != StopAt; ++I) {
    ...
    if (const CallInst *CI = dyn_cast<CallInst>(I)) {
      // Cannot duplicate noduplicate or convergent calls.
      if (CI->cannotDuplicate() || CI->isConvergent())
        return InstructionCost::getInvalid();
    }

    Size += TTI->getUserCost(&*I, TargetTransformInfo::TCK_SizeAndLatency);
  }
...
bool JumpThreadingPass::maybethreadThroughTwoBasicBlocks(BasicBlock *BB,
  ...
  InstructionCost BBCost = getJumpThreadDuplicationCost(
      TTI, BB, BB->getTerminator(), BBDupThreshold);
  InstructionCost PredBBCost = getJumpThreadDuplicationCost(
...
bool JumpThreadingPass::tryThreadEdge(
  ...
  InstructionCost JumpThreadCost = getJumpThreadDuplicationCost(
      TTI, BB, BB->getTerminator(), BBDupThreshold);
...
bool JumpThreadingPass::duplicateCondBranchOnPHIIntoPred(
  ...
  InstructionCost DuplicationCost = getJumpThreadDuplicationCost(
      TTI, BB, BB->getTerminator(), BBDupThreshold);
...
bool JumpThreadingPass::threadGuard(BasicBlock *BB, IntrinsicInst *Guard,
  ...
  InstructionCost Cost = getJumpThreadDuplicationCost(
      TTI, BB, AfterGuard, BBDupThreshold);
