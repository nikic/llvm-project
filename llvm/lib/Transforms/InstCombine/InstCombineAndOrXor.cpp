...
Value *InstCombinerImpl::foldAndOfICmps(ICmpInst *LHS, ICmpInst *RHS,
  ...
  // Check that the intersection is precise and not an approximation.
  // TODO: Sink this into ConstantRange.
  if (CR != CR1.inverse().unionWith(CR2.inverse()).inverse())
    return nullptr;
...
Value *InstCombinerImpl::foldOrOfICmps(ICmpInst *LHS, ICmpInst *RHS,
  ...
  ConstantRange CR2 =
      ConstantRange::makeExactICmpRegion(PredR, RHSC->getValue());
  ConstantRange CR = CR1.unionWith(CR2);

  // Check that the union is precise and not an approximation.
  // TODO: Sink this into ConstantRange.
  if (CR != CR1.inverse().intersectWith(CR2.inverse()).inverse())
    return nullptr;
