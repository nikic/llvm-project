...
void llvm::CloneAndPruneIntoFromInst(Function *NewFunc, const Function *OldFunc,
  ...
  // We try to fold constant terminators and remove unreachable blocks during
  // cloning, but phi simplifications only happen after that. Remove unreachable
  // blocks again here to account for that.
  SmallVector<BasicBlock *, 16> BBs;
  Function::iterator Begin = cast<BasicBlock>(VMap[StartingBB])->getIterator();
  for (BasicBlock &BB : make_range(Begin, NewFunc->end()))
    BBs.push_back(&BB);
  removeUnreachableBlocks(&*Begin, BBs);
