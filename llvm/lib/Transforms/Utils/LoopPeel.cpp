...
bool PhiAnalyzer::isInductionPHI(const PHINode *Phi) const {
  ...
  // Start at the incoming value of the phi and traverse definitions. We
  // consider the phi to be an IV if we can return to it again by traversing
  // only add, sub, or cast instructions.
  while (true) {
    if (Cur == Phi)
      break;
