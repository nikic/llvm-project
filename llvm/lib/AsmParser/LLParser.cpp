...
bool LLParser::validateEndOfModule(bool UpgradeDebugInfo) {
  ...
  for (const auto &RAG : ForwardRefAttrGroups) {
    ...
    for (const auto &Attr : Attrs) {
      auto It = NumberedAttrBuilders.find(Attr);
      if (It != NumberedAttrBuilders.end())
        B.merge(It->second);
    }

    if (Function *Fn = dyn_cast<Function>(V)) {
      AttributeList AS = Fn->getAttributes();
      AttrBuilder FnAttrs(Context, AS.getFnAttrs());
      ...
      AttrBuilder Attrs(Context, GV->getAttributes());
      Attrs.merge(B);
...
bool LLParser::parseUnnamedAttrGrp() {
  ...
  AttrBuilder &B = NumberedAttrBuilders
    .emplace(VarID, AttrBuilder(Context)).first->second;
  if (parseFnAttributeValuePairs(B, unused, true, BuiltinLoc) ||
...
bool LLParser::parseInvoke(Instruction *&Inst, PerFunctionState &PFS) {
  ...
  AttrBuilder RetAttrs(Context), FnAttrs(Context);
  std::vector<unsigned> FwdRefAttrGrps;
  LocTy NoBuiltinLoc;
  unsigned CC;
